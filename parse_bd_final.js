...
for (let i = 0; i < lines.length; i++) {
  ...
  // Check for "Правильный ответ:" line
    ...
    // Extract correct answer(s) from the line or next lines
    ...
    // Also check next few lines for answer patterns
    for (let j = 1; j <= 5; j++) {
      if (i + j < lines.length) {
        const nextLine = lines[i + j].trim();
        const nextAnswerMatch = nextLine.match(/\(([A-Z])\)/);
        if (nextAnswerMatch) {
          currentCorrectAnswer = nextAnswerMatch[1];
          break;
        }
      }
    }
